...
class interface(wx.Frame):
    ...
    def takePhoto(self, event):
        ...
        while (cap.isOpened()):
            ...
            try:
                ...
                # wx.CallAfter(self.show_image)
                # self.show_image(image)
                cv2.imshow('img', image)
            #
            #     height, width, channel  = image.shape
            #     pic = wx.BitmapFromBuffer(width, height, image)
    ...
    def photo_sure(self, event):###拍照确定 1.关闭摄像头 2.保存特征和id 3 关闭线程 4 注册界面
        self.cap.release()
        self.save_id(self.im_rd)
        self.set_button()
        # _thread.exit()

    def photo_delay(self, event):    ###1.关闭摄像头 2.关闭线程 3.注册界面
...
class App(wx.App):
    ...
    def load_model(self):
        ...
        parser.add_argument('--model', default='../recognition/models/y2-iccv/model, 01',
